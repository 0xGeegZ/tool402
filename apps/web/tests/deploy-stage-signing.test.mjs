...
implementedTest("composes the accepted wallet island, signature dialog, and stage list exactly once", async () => {
  ...
  assert.match(island, /worldVerifiedAddress === session\?\.address/u, "World verification must stay bound to the connected issuer address");
  assert.match(island, /kind: "unavailable"/u, "directory publication must be rendered unavailable until World verification completes");
});

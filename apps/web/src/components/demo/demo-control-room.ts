...
export const recordingSteps: readonly RecordingStep[] = Object.freeze([
  ...
  { id: "consumer-agent", href: "/demo", title: "Show Consumer Agent payment evidence", wallet: "Human Ops payer", do: "Run the safe preflight. Use an existing verified payment instead of paying again.", say: "The Consumer Agent checks the exact quote before one authorized paid retry.", show: "Safe terminal lines and a verified settlement link when one exists." },
  ...
  { id: "ats-deployment", href: "/provider/deploy", title: "Prepare the ATS deployment", wallet: "PROVIDER", do: "Continue only after the real stage result is observed.", say: "The ATS asset is only verified after the independent receipt path accepts it.", show: "Pending state, or verified asset evidence if available." },
  { id: "ats-lifecycle", href: "/demo", title: "Show the ATS lifecycle operation", wallet: "ATS issuer", do: "Use the approved one-unit transfer only after its preconditions are verified.", say: "We show the asset state before and after one bounded lifecycle operation.", show: "Verified pre-state, transaction, and post-state when available." },
  { id: "world", href: "/demo", title: "World verification", wallet: "PROVIDER", do: "Show World only when the current wallet has a valid verified proof.", say: "A QR code is not verification. The proof must be valid for this wallet and session.", show: "Verified state, or the honest optional omission." },
  ...
  { id: "backing", href: "/explore/riskscan/back", title: "Back RiskScan", wallet: "BACKER", do: "Use the existing amount and read the acknowledgement before choosing it.", say: "The Backer signs intent first and explicitly submits the HBAR transfer second.", show: "Units, HBAR amount, unchecked acknowledgement, then allocation pending after submission." },
  ...
  { id: "dashboard", href: "/dashboard", title: "Show repeatability", wallet: "PROVIDER", do: "Open the signed dashboard.", say: "The dashboard restores the campaign linked to the signed session without recreating it.", show: "The current campaign or honest empty state." },

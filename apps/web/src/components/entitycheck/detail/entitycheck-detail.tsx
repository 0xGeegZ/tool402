...
import type { EntityCheckDisposition, EntityCheckScreen } from "@tool402/core";
...
const inputFields = [
  ...
  [
    "registrationNumber",
    "Optional. Exactly nine digits, the SIREN that disambiguates a name.",
  ],
] as const;
...
const dispositions = [
  [
    "found",
    "Exactly one registry candidate resolved, or one candidate matched the supplied SIREN.",
  ],
  [
    "ambiguous",
    "Two or more candidates and no SIREN match; the first five pairs are listed.",
  ],
...
const screens = [
  ...
  [
    "hit",
    "At least one entry matched; every match is listed in dataset order.",
  ],
  ["not_screened", "No screen ran because no single entity was resolved."],
] as const satisfies readonly [EntityCheckScreen, string][];

const sources = [
  [
    "FR_RECHERCHE_ENTREPRISES",
    "The French company registry search API (recherche-entreprises), read once per request.",
  ],
  [
    "OFAC_SDN",
    "The US Treasury OFAC Specially Designated Nationals list, read at most once a day and cited by content hash.",
  ],
] as const;
    ...
    <div className="space-y-3 py-5">
      ...
      <dl className="space-y-3">
        {fields.map(([field, description]) => (
          <div
            key={field}
            className="space-y-1 sm:grid sm:grid-cols-[12rem_1fr] sm:gap-4 sm:space-y-0"
          >
  ...
  return (
    <article className="mx-auto max-w-3xl space-y-8">
      <div className="space-y-6">
        ...
        <header className="space-y-3">
          ...
          <p className="text-lg leading-8 text-muted-foreground">
            A bounded lookup that resolves a French company query to one public
            registry record and screens its legal name against a public
            sanctions list.
          </p>
        </header>
      </div>
      ...
      <section aria-labelledby="entitycheck-capability" className="space-y-3">
        <h2
          id="entitycheck-capability"
          className="text-2xl font-semibold tracking-tight"
        >
          Capability
        </h2>
        <p className="leading-7 text-muted-foreground">
          One request resolves a name or SIREN to a registry record with its
          legal name, administrative status, incorporation date, registered
          address, and officer count, then reports whether that legal name
          appears on the sanctions list. Nothing is inferred beyond the two
          records read.
        </p>
      </section>
      ...
      <section aria-labelledby="entitycheck-inputs" className="space-y-4">
        <div className="space-y-2">
          <h2
            id="entitycheck-inputs"
            className="text-2xl font-semibold tracking-tight"
          >
            Inputs
          </h2>
          <p className="leading-7 text-muted-foreground">
            EntityCheck accepts the following request fields.
          </p>
        </div>
        <Card>
      ...
      <section aria-labelledby="entitycheck-results" className="space-y-4">
        <div className="space-y-2">
          <h2
            id="entitycheck-results"
            className="text-2xl font-semibold tracking-tight"
          >
            Result boundary
          </h2>
          <p className="leading-7 text-muted-foreground">
            Every result carries one disposition and one sanctions screen value,
            without a score.
          </p>
        </div>
      ...
      <section aria-labelledby="entitycheck-sources" className="space-y-4">
        <div className="space-y-2">
          <h2
            id="entitycheck-sources"
            className="text-2xl font-semibold tracking-tight"
          >
            Sources
          </h2>
          <p className="leading-7 text-muted-foreground">
            Both sources are public and each result cites the descriptor of the
            record it used.
          </p>
        </div>
        <Card>

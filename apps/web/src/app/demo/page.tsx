    ...
    <main className="pb-6 sm:pb-12">
      <article className="mx-auto max-w-5xl space-y-8">
        <PageHeader
          ...
          description="A presenter-first route through discovery, the 402 payment boundary, and a bounded result. Keep this guide open while you present."
        />
        <section aria-label="Demo context" className="grid gap-3 sm:grid-cols-4">
          ...
          <div className="rounded-2xl border border-border bg-card p-4"><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Core story</p><p className="mt-2 font-semibold">Discover → pay → result</p></div>
        ...
        <section aria-labelledby="demo-loop-title" className="rounded-3xl border border-brand-purple/20 bg-brand-purple/5 p-5 sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div><p className="text-xs font-semibold uppercase tracking-wide text-brand-purple">The story to tell</p><h2 id="demo-loop-title" className="mt-1 text-xl font-bold">Discover → Request → 402 → Settlement → Bounded result</h2></div>
            <p className="max-w-sm text-sm leading-relaxed text-muted-foreground">The guide is static by design. Only claim a payment, deployment, or result when that capability is visibly configured and verified.</p>
          </div>
        </section>
        <GuidedDemoSteps />
      </article>

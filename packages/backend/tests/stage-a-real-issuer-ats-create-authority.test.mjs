...
function staticModuleSpecifiers(source) {
  ...
  const prohibitedRuntimeIndirectionNodes = [];
  const inspectNode = (node) => {
    if (
      ...
      prohibitedRuntimeIndirectionNodes.push(
    ...
    if (
      ...
      prohibitedRuntimeIndirectionNodes.push(node.text);
    }
    typescript.forEachChild(node, inspectNode);
  };
  typescript.forEachChild(sourceFile, inspectNode);
  assert.deepEqual(prohibitedRuntimeIndirectionNodes, []);
...
test("returns the exact planned authority and closed real-owner configuration", () => {
  ...
  assert.equal(
    keccak256(
      stringToHex(
        canonicalizeRequirements(preimageFor(projection.atsCreateConfiguration)),
      ),
    ).slice(2),
...
test("returns fresh fully frozen authority and configuration data on every call", () => {
  ...
  assert.notStrictEqual(
    first.atsCreateConfiguration,
    second.atsCreateConfiguration,
  );
  assert.notStrictEqual(

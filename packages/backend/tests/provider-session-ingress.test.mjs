...
implementedTest("forwards a signed self-service ensure assertion only to the private membership mutation", async () => {
  const { handleProviderSessionIngressForTest } = await import(ingressUrl.href);
  const { request, key } = await signedRequest(JSON.stringify({
    type: "self_service_ensure", canonicalSignerAddress,
    sessionExpiresAt: "2026-01-01T08:00:00.000Z",
  }));
  const calls = [];
  const response = await handleProviderSessionIngressForTest({}, request, {
    nowMilliseconds: () => 1_735_689_600_000,
    ...
    deployment: async () => { throw new Error("must not read deployment"); },
    backing: async () => { throw new Error("must not back"); },
    backingReserve: async () => { throw new Error("must not reserve"); },
    backingRead: async () => { throw new Error("must not read backing"); },
    ensureSelfService: async (input) => { calls.push(input); return { outcome: "ACTIVE" }; },
  });
  assert.equal(response.status, 200);
  assert.deepEqual(await response.json(), { outcome: "ACTIVE" });

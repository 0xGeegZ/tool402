...
| UI-S04 | Local manifest recorded; adaptation pending | Browser ToolLoop route, current-origin Agent composition, opaque terminal state rendering, and one local detail link | Direct endpoint/header/payment handling, runtime configuration, wallet/signer/provider/account surfaces, price/network/recipient/facilitator, result/receipt/evidence, persistence, retry/storage, external links, fabricated completion, and full-tree import |

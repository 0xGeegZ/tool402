...
| UI-S03 | Local adaptation in module review; not accepted | Try RiskScan route, typed browser response mapping, bounded form submission, and local detail-to-Try navigation | Runtime configuration reads, wallet/signer/payment-header authoring, payment payload display, direct facilitator access, provider/account surfaces, price, receipt, evidence, fabricated result/payment/completion state, external links, analytics, persistence, and full-tree import |

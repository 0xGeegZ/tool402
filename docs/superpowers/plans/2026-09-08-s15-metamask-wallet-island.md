...
is refused without buffering it whole. Because the relay cannot tell the
browser which side refused, the dialog's `REJECTED` copy says "before or at
the backend" and never attributes a reason to the backend.

The sign-and-relay sequence lives in `command-relay.ts` as
`signAndRelayCommand`, so a stub provider can prove the call order, the fresh
nonce per attempt, and that nothing is relayed when signing fails; the dialog
only maps its closed result to a phase and a message. A request whose
`expiresAt` has passed is refused before any wallet read, because the payload
binds the timestamps and only the composing route can issue a fresh request.

...
- Owner: the proposed implementation scope is `packages/backend/convex/riskscan_pending_settlement_reader.ts` and `packages/backend/tests/risk-scan-pending-settlement-reader.test.mjs`. The root owns this card, its local specification and plan, queue state, catalog, file ownership, decisions, and integration evidence.

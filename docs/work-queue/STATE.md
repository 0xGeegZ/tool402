...
- CURRENT_TASK: M32-T010 (00-inbox); corrected authority and a fresh independent
  review are required before ready
- ACTIVE_LANES: none
...
- NEXT_ELIGIBLE_TASKS: M32-T010 is the sole inbox CORE_P0 card. Its internal
  ...
  authority/version and apply accepted replay/idempotency semantics. It cannot
  claim M25/M31 process-local provenance across a serialized invocation. A
  later BFF adapter must establish that provenance and define any branded
  asynchronous completion, timeout, and recovery protocol. M32 must not
  ...
  reconciliation.
- LAST_QUEUE_VALIDATION: Two independent authority/plan reviews at pushed
  `cb6164b5520dd3f598f97d6a092142fa79c973e3` found missing serialized rebinding,
  full time validation, closed return contracts, fresh-nonce consumption, and
  exact RiskScan-namespace coverage. D-M32-010-002 records the bounded
  correction from accepted human authority. M32-T010 remains in 00-inbox until
  the corrected committed authority receives a fresh clean review; no RED/code
  is authorized.

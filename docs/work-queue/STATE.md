...
- CURRENT_TASK: M04-T030 (20-active)
- ACTIVE_LANES: M04-T030 Convex request writer
...
- NEXT_ELIGIBLE_TASKS: none while M04-T030 owns its disjoint backend paths. The active task begins from the recorded RED contract and remains internal writer code only.
- LAST_QUEUE_VALIDATION: M04-T030 passed the ready-state dependency, ownership, local specification/plan, human-boundary, and queue checks, then a fresh activation rescan. It must not be represented as a configured or live durable store, payment/settlement, verification, evidence, or result.

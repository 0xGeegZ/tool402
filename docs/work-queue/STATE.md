...
- NEXT_ELIGIBLE_TASKS: Run a fresh post-M07 rescan before recording the next local CORE_P0 card. Prioritize the remaining user-visible local vertical; do not reopen M04 persistence or reconciliation merely because its former queue lanes are complete. Do not create a payment client or live path while HA-X402-HEDERA-001 is PENDING.
- LAST_QUEUE_VALIDATION: M07-T010 is accepted as a controlled local unsigned composition after root validation and two clean module-review generations. No real recipient/facilitator configuration, wallet/account/key action, payment, transaction, deployment, or live claim is authorized.

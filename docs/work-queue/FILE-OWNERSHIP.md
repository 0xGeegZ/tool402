...
M02-T010 and M02-T020 are accepted delivery records. M02-T030 reserves its card, backend projection source/tests, and its minimum local projection contract. M02-T040 reserves its card, landing/Explore routes, `apps/web/src/components/landing/**`, `apps/web/src/components/discovery/**`, focused UI-S01 tests, and its minimum local UI record. The root alone owns package metadata, the lockfile, queue state, local UI ledgers, and integration evidence.

...
S36-T010 is an `00-inbox` root-owned POLISH intake. It owns its card,
UI-S36 manifest, implementation plan, catalog, state, ledger, decision, and
future review evidence. Its candidate Web paths are only
`apps/web/src/components/provider/deploy/provider-deploy-stages.tsx` and new
`apps/web/tests/provider-deploy-signature-handoff.test.mjs`. Neither path is
reserved or authorized until a fresh readiness and separate durable RED
activation. S26/S27 remain inbox-only and M50/S29 are accepted with no active
reservation; command, wallet/provider, relay, authority, API, payment,

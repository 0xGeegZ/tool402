...
M56-T010 is a root-owned `00-inbox` CORE_P0 backing-demo successor. It owns control records, new server projection helper, back page, RiskScan detail entry point, existing `backing-flow.tsx`, named focused tests, and smallest authority schema/normalizer/admission paths after readiness/RED activation. The delegated lane may alter only `backing-state.ts` plus its new named test; it cannot overlap S26's backing-flow mount or route assertions. No allocation, candidate, package, lockfile, or live action is reserved.

...
M02-T070 is an inbox-only candidate with its card, docs/ui/UI-S02.md, docs/ui/IMPORT-LEDGER.md, and docs/superpowers/plans/2026-09-05-m02-ui-s02-riskscan-detail.md reserved to the root. On activation, its implementation lane may own `apps/web/src/app/explore/riskscan/page.tsx`, `apps/web/src/components/riskscan/detail/**`, `apps/web/src/components/discovery/riskscan-discovery-card.tsx`, `apps/web/tests/riskscan-detail.test.mjs`, and `apps/web/tests/landing-explore.test.mjs`; the root continues to own queue records and shared UI ledger state.

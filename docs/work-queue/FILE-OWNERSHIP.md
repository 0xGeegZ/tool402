...
M03-T010 is queued for pure core receipt/evidence binding. Its implementation lane will own `packages/core/src/risk-scan.ts`, `packages/core/src/index.ts`, and `packages/core/test/risk-scan.test.mjs`. The root owns its card, local specification, plan, queue state, catalog, decisions, and integration evidence. The accepted backend projection, API route, server helper, browser flow, package metadata, lockfile, runtime configuration, and external resources are not owned by this card.

...
S48-T010 is inbox-only. Its proposed source/test boundary is
...
specification, UI manifest, catalog, ledger, state, and root records. No
source or test is reserved before separate readiness and RED activation. The
S26 header and shell wrapper in `layout.tsx` remain excluded.
...
User-directed root environment-template parity amendment: the root additionally
reserves `.env.example` only to add the eight root-local names missing at
`origin/dev`: `TOOL402_CONVEX_SITE_URL`, `TOOL402_INGRESS_KEY_ID`,
`TOOL402_INGRESS_SECRET`, `WORLD_APP_ID`, `WORLD_RP_ID`,
`WORLD_RP_SIGNING_KEY`, `WORLD_ACTION`, and `WORLD_ENVIRONMENT`. Each is an
empty documentation placeholder; no local value, runtime configuration, TLS
trust, wallet action, signature, relay, transaction, deployment, or live

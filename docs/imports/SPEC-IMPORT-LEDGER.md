...
| `docs/specs/m32-durable-external-prepare-admission.md` | Runtime-local | Separate internal Convex data plane for current-authority recheck, replay/idempotency admission, one generic prepared attempt, and exact recovery; ATS target/hash remain opaque signed context | Contract intake and independent design review required before RED |

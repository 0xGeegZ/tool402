...
| `docs/specs/m33-ats-prepare-authority-gate.md` | Runtime-local | Zero-enabled compiled server-only ATS manifest that rejects all current ATS external-prepare candidates before M32 durable replay/idempotency access | Contract reviewed and ready for local implementation |

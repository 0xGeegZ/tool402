...
| `docs/specs/m33-ats-prepare-authority-gate.md` | Runtime-local | Zero-enabled compiled server-only ATS manifest that rejects all current ATS external-prepare candidates before M32 durable replay/idempotency access | Test-only RED accepted; declared private resolver and M32 assertion are complete and under acceptance review |

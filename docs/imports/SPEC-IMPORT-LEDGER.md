...
| `docs/specs/m31-external-prepare-command-admission.md` | Runtime-local | M30-authenticated same-process atomic-admission handoff with no durable storage, generic attempt, ATS authority, or external behavior | Clarified synchronous data-port authority reviewed and ready for root activation |
